...
3. Набор параметров выходных результатов генерации - блок `OutputParameters`
   ...
   9. `save_circuit_parameters_json` (bool, опциональный, true) - при `true` выполняется обсчет параметров схемы и рядом с Verilog сохраняется файл `<имя_схемы>.json`; при `false` генерируются только Verilog и прочие форматы по флагам выше, без JSON с параметрами схемы
