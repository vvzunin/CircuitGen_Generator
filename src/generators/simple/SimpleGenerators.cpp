...
SimpleGenerators::getRandomElement(const GatesInfo &i_info) {
  // rand element of map
...
std::pair<Gates, int32_t>
SimpleGenerators::getRandomElement(uint32_t i_gatesLimit) {
...
GraphPtr SimpleGenerators::cnfFromTruthTable(const TruthTable &i_table,
                                             bool i_tp) {
...
GraphPtr
SimpleGenerators::generatorNumOperation(uint32_t i_input, uint32_t i_output,
...
SimpleGenerators::delNull(std::map<Gates, int32_t> i_copyLogicOper) {
...
GraphPtr SimpleGenerators::generatorSummator(uint32_t i_bits, bool i_overflowIn,
...
GraphPtr SimpleGenerators::generatorComparison(uint32_t i_bits, bool compare0,
...
GraphPtr SimpleGenerators::generatorSubtractor(uint32_t i_bits,
                                               bool i_overflowIn,
...
GraphPtr SimpleGenerators::generatorALU(
    ...
    bool NSUB, bool MULT, bool COM, bool AND, bool NAND, bool OR, bool NOR,
    bool XOR, bool XNOR, bool CNF, bool RNL, bool NUM_OP, uint32_t minLevel,
    uint32_t maxLevel, uint32_t minElement, uint32_t maxElement,
    std::map<Gates, int32_t> m, bool LeaveEmptyOut) {
  GraphPtr graph(new OrientedGraph);
  ...
  if (RNL) {
    auto lambda = [&]() {
      ...
      auto curGraph = generatorRandLevel(minLevel, maxLevel, minElement,
                                         maxElement, i_bits, i_outbits);
  ...
  for (size_t i = 0; i < inputs_alu.size(); i++) {
    inputs_alu[i].insert(inputs_alu[i].begin(), controls.begin(),
                         controls.end());
  }
