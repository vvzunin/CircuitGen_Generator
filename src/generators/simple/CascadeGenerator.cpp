...
GraphPtr CascadeGenerator::generatorCascade() {
  ...
  if (m_minStates > m_maxStates) {
    throw std::invalid_argument(
        "min_num_states cannot be greater than max_num_states");
  }
  ...
  for (uint32_t automaton_idx = 0; automaton_idx < m_numautomatons;
       ++automaton_idx) {
    d_randGenerator.setSeed(q);
    q += q;
    bool isMoore = d_randGenerator.getRandInt(0, 1, true);
    uint32_t numStates =
        d_randGenerator.getRandInt(m_minStates, m_maxStates, true);
    ...
    for (uint32_t i = 0; i < numOutputs; ++i) {
      VertexPtr output = graph->addOutput("a" + std::to_string(automaton_idx) +
                                          "_f" + std::to_string(i));
      VertexPtr or_gate =
          graph->addGate(Gates::GateOr, "a" + std::to_string(automaton_idx) +
                                            "_or_f" + std::to_string(i));
      graph->addEdge(or_gate, output);
      output_or_gates.push_back(or_gate);
      automaton_outputs.push_back(output);
    }

    std::unordered_map<std::string, VertexPtr> state_bufs, state_ors,
        state_dffs;
    for (uint32_t i = 0; i < numStates; ++i) {
      std::string name =
          "a" + std::to_string(automaton_idx) + "_s" + std::to_string(i);
    ...
    for (const auto &[fromName, fromBuf]: state_bufs) {
      for (uint32_t i = 0; i < automaton_inputs.size(); ++i) {
        std::string toName = "a" + std::to_string(automaton_idx) + "_s" +
                             std::to_string(stateDist(gen));
        if (reachable_states.size() < numStates) {
          for (uint32_t idx = 0; idx < numStates; ++idx) {
            std::string candidate = "a" + std::to_string(automaton_idx) + "_s" +
                                    std::to_string(idx);
        ...
        VertexPtr and_gate =
            graph->addGate(Gates::GateAnd, fromName + "_to_" + toName + "_x" +
                                               std::to_string(i));
    ...
    for (const auto &[stateName, _]: state_bufs) {
      if (reachable_states.count(stateName) == 0) {
        ...
        VertexPtr and_gate = graph->addGate(
            Gates::GateAnd, fromName + "_to_" + stateName + "_fix");
        graph->addEdge(fromBuf, and_gate);
    ...
    if (isMoore) {
      ...
      for (uint32_t out_idx = 0; out_idx < numOutputs; ++out_idx) {
        ...
        do {
          ...
          state_name = "a" + std::to_string(automaton_idx) + "_s" +
      ...
      for (const auto &[state_name, state_buf]: state_bufs) {
        if (used_states.count(state_name) == 0) {
          uint32_t out_idx =
              d_randGenerator.getRandInt(0, numOutputs - 1, true);
          graph->addEdge(state_bufs[state_name], output_or_gates[out_idx]);
        }
      }
    } else {
      ...
      for (const auto &[state_name, state_buf]: state_bufs) {
        for (VertexPtr out_conn: state_buf->getOutConnections()) {
          if (out_conn->getType() == VertexTypes::gate &&
              static_cast<GraphVertexGates *>(out_conn)->getGate() ==
                  Gates::GateAnd) {
      ...
      for (uint32_t out_idx = 0; out_idx < numOutputs; ++out_idx) {
        ...
        do {
          uint32_t trans_idx =
              d_randGenerator.getRandInt(0, all_transitions.size() - 1, true);
    ...
    VertexPtr const1 =
        graph->addConst('1', "a" + std::to_string(automaton_idx) + "_init");
    VertexPtr initAnd = graph->addGate(
        Gates::GateAnd, "a" + std::to_string(automaton_idx) + "_init_and");
    VertexPtr initOr = graph->addGate(
        Gates::GateOr, "a" + std::to_string(automaton_idx) + "_init_or");
    ...
    VertexPtr init_dff = graph->addSequential(
        static_cast<SequentialTypes>(ASYNC | ff | RST), clock, initOr, reset,
        "init_dff_" + std::to_string(automaton_idx));
