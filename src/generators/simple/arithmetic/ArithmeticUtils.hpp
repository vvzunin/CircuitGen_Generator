...
struct ArithemticOperations {
  enum Types : uint8_t { UNDEFINED = 0u, MUL, DIV, ADD, SUB, NEG, INC, DEC };
  ...
  inline static Types getArithmeticType(const std::string_view &i_name) {
    static const std::pair<std::string_view, Types> names[] = {
        {getName(MUL), MUL}, {getName(DIV), DIV}, {getName(ADD), ADD},
        {getName(SUB), SUB}, {getName(NEG), NEG}, {getName(INC), INC},
        {getName(DEC), DEC}};
    auto lambda =
        [&i_name](const std::pair<std::string_view, Types> &type) -> bool {
      return type.first == i_name;
    };
    const auto *iter = std::find_if(std::begin(names), std::end(names), lambda);
...
commonPrefixOperations(GraphPtr graph, const std::vector<VertexPtr> &word,
                       uint32_t width, bool signExtend,
