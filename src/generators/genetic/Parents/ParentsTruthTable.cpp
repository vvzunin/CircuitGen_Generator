...
{
  int count = 0;
  std::vector<std::vector<bool>> p1 = i_population[i_t].getChromosomeType().getOutTable();
  ...
  for (int i = 0; i < i_population.size(); ++i)
  {
    ...
    {
      count = 0;
      std::vector<std::vector<bool>> p2 = i_population[i].getChromosomeType().getOutTable();
