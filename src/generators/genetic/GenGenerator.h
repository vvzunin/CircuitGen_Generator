...
void GeneticGenerator<Type, ParametersType>::createPopulation()
{
    ...
    for (int i = 0; i < d_parameters.getPopulationSize(); ++i)
    {
        ...
        ChromosomeType<Type, ParametersType> ind("ind" + std::to_string(i), gen);
...
template<typename Type, typename ParametersType>
std::vector<ChromosomeType<Type, ParametersType>> GeneticGenerator<Type, ParametersType>::generate()
{   
    createPopulation();
    ...
    {
        std::vector<ChromosomeType<Type, ParametersType>> newPopulation = RecombinationType<Type, ParametersType>(d_parameters.getRecombinationParameters(), d_population);
        std::vector<ChromosomeType<Type, ParametersType>> mutants = MutationType<Type, ParametersType>(d_parameters.getMutationParameters(), newPopulation);
