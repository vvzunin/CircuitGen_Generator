...
TEST(GenerateDataBaseRandLevel, EqualWithTheSameSeeds) {
  ...
  for (size_t i = 0; i < jsons1.size(); ++i) {
    EXPECT_EQ(FilesTools::loadStringFile(jsons1[i]),
              FilesTools::loadStringFile(jsons2[i]));
  }
...
TEST(generateDataBaseNumOperations, EqualWithTheSameSeeds) {
  ...
  std::map<Gates, int> logicOper = {{Gates::GateAnd, 5},  {Gates::GateOr, 5},
                                    {Gates::GateNot, 6},  {Gates::GateNor, 6},
                                    {Gates::GateNand, 1}, {Gates::GateXnor, 9},
                                    {Gates::GateXor, 12}};
