...
TEST(MutationTruthTableTest, MutationBinary) {
  ...
  ChronosomeType<TruthTable, TruthTableParameters> chronosome("Test", table);
  population.push_back(chronosome);
  ...
  ASSERT_NE(
      mutatedPopulation[0].getChronosomeType().getOutTable(),
      table.getOutTable()
  );
}
...
mutatedTable = mutatedPopulation[0].getChronosomeType().getOutTable(); bool
