...
TEST(FileTools, OnlyDirectoriesNoFiles) {
  fs::path tmpPath =
      (fs::temp_directory_path() /
       "test"); // Created temporary guaranteed directory to deal with
  std::string
  ...
  fs::create_directories(
      tmpPath / "6"); // Just created directories within temporary directory .
                      // The order does not matter. You will see below why
  ...
  std::vector<std::string> correctVec = {
      "1", "2", "3",
      "4", "5", "6"}; // Created vector that's going to be compared
                      // with the result vector we want to test

  tmp = tmpPath.string(); // get the path to the current directory which contain
                          // created directories
  ...
  std::sort(
      correctVec.begin(),
      correctVec
          .end()); // If the elements of correctVec and resVec are the same
  std::sort(
      resVec.begin(),
      resVec.end()); // after sort they are both will be equal to each other.

  EXPECT_EQ(resVec, correctVec);

  std::filesystem::remove_all(
      tmpPath.string()); // Delete the current directory we work with.
}
TEST(FileTools, NoDirectoriesNoFiles) {
  fs::path tmpPath =
      (fs::temp_directory_path() /
       "anotherOne"); // Created temporary guaranteed directory to deal with
...
TEST(FileTools, NoDirectoriesOnlyFiles) {
  ...
  std::string tmp = tmpPath.string(); // to prevent some errors  about rvalue to
                                      // non-const lvalue and so on
  ...
  for (int32_t i = 1; i <= 10; i++) {
    ...
    file.open((tmpPath / nameTxtFile).string()); // Just create a file with name
                                                 // i.txt in current directory
    file.close();
  }
...
TEST(FileTools, DirectoriesAndFilesExist) {
  ...
  for (int32_t i = 1; i <= 10; i++) {
    ...
    const char *nameTxtFile(str.c_str());
    file.open(
        (tmpPath / nameTxtFile).string()); // Just create a file with name i.txt
    file.close();
  }
