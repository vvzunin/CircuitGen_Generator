...
protected:
  ...
  void TearDown() { vertex = std::make_shared<GraphVertex>("or", "or"); }
...
TEST_F(GraphVertexTest, setLevelAndGetLevelWorksCorrectly) {
  ...
  for (int i = 0; i < 5; i++)
    vecOfThreads.push_back(
        std::move(std::thread(Loop<GraphVertex>::loop, i, vertexes[i]))
    );
  for (int i = 0; i < 5; i++)
    vecOfThreads[i].join();
}
...
TEST_F(GraphVertexTest, setLogicExpressionAndGetLogicExpressionWorksCorrectly) {
  for (int i = 0; i < 50; i++) {
    ...
    ASSFERT_EQ(vertex->getLogicExpression(), std::to_string(i));
  }
}
