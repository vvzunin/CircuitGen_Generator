...
TEST(GetStatsTest, ErrorWithInvalidLibraryName) {
  ...
  auto        result  = util.getStats(
      "correct_filename.v",
      libName,
      "../../test_data/OpitmizationUtilits",
      libPath
  );
  std::string errorText = "Incorrect read: Cannot open input file \"" + libPath
                        + "/" + libName + "\". \n\n";
...
TEST(GetStatsTest, ErrorWithInvalidFileName) {
  ...
  result = util.getStats(
      "incorrect_filename.v",
      "sky130.lib",
      "../../test_data/OpitmizationUtilits",
      "../../../libs"
  );
...
TEST(GetStatsTest, ErrorWithInvalidFilePath) {
  ...
  result = util.getStats(
      "correct_filename.v", "sky130.lib", "incorrect_filepath", "../../../libs"
  );
  readFlag = result.commandsOutput["fileRead"];
